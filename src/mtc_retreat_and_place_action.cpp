...
private:
  ...
  void execute_goal(const std::shared_ptr<GoalHandle> goal_handle)
  {
    ...
    try {
      ...
      goal_handle->abort(std::make_shared<RetreatAndPlace::Result>(result));
      return;
    }
    ...
    if (goal_handle->is_canceling()) {
      ...
      goal_handle->canceled(std::make_shared<RetreatAndPlace::Result>(result));
      return;
    }
    ...
    try {
      ...
      goal_handle->succeed(std::make_shared<RetreatAndPlace::Result>(result));
      return;
    }
    ...
    result.success = true; result.error_code = 0; result.message = "Retreat and Place succeeded.";
    goal_handle->succeed(std::make_shared<RetreatAndPlace::Result>(result));
